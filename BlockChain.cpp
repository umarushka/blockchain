...
#define MAX_BIG_NUMBER 256 // this code snippet only supports big nubmers up to 256 bytes in length
    ...
    bool CBDecodeBase58(BigNumber * bi, const char * str) {
        ...
        for (uint32_t x = slen - 1;; x--) { // Working backwards
...
const char * getAgeString(AgeMarker m) {
    ...
    switch (m) {
        case AM_ONE_DAY: ret = "One Day";
            break;
        case AM_ONE_WEEK: ret = "One Week";
            break;
        case AM_ONE_MONTH: ret = "One Month";
            break;
        case AM_THREE_MONTHS: ret = "1-3 Months";
            break;
        case AM_SIX_MONTHS: ret = "3-6 Months";
            break;
        case AM_ONE_YEAR: ret = "Six Months to One Year";
            break;
        case AM_TWO_YEARS: ret = "One to Two Years";
            break;
        case AM_THREE_YEARS: ret = "Two to Three Years";
            break;
        case AM_FOUR_YEARS: ret = "Three to Four Years";
            break;
        case AM_FIVE_YEARS: ret = "Four to Fix Years";
            break;
    }
    return ret;
}
...
public:
...
#else
        sprintf(scratch, "%s%s", mRootDir, files.c_str()); // get the filename
#endif
    ...
    virtual void printJSON(const Block *block) // prints the contents of the block to the console for debugging purposes
    {
        static json j;

        j = {
            {"Block", formatNumber(block->blockIndex)},
            {"Number of Transactions", formatNumber(block->transactionCount)},
            {"Timestamp", getTimeString(block->timeStamp)},
            {"Bits", block->bits},
            {"Size, KB", (float) block->blockLength / 1024.0f},
            {"Version", block->blockFormatVersion},
            {"Nonce", block->nonce},
            {"BlockReward", (float) block->blockReward / ONE_BTC}
        };
